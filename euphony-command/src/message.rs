...
pub trait Handler {
    ...
    fn spawn_sink(&mut self, msg: SpawnSink) -> io::Result<()>;
    ...
    fn finish_sink(&mut self, msg: FinishSink) -> io::Result<()>;
}
...
pub fn decode_one<R: io::Read, H: Handler>(
    ...
    match tag {
        ...
        SpawnSink::TAG => {
            let msg = SpawnSink::decode(tag, input)?;
            handler.spawn_sink(msg)?;
        }
        ...
        FinishSink::TAG => {
            let msg = FinishSink::decode(tag, input)?;
            handler.finish_sink(msg)?;
        }
        _ => {
...
impl Codec for CreateGroup {
    ...
    fn decode<R: io::Read>(tag: u8, input: &mut R) -> io::Result<Self> {
        ...
        let name = if len > 0 {
            ...
            String::from_utf8(name).unwrap_or_else(|_| "INVALID GROUP NAME".to_string())
        } else {
            String::new()
        };
        Ok(Self { id, name })
...
impl fmt::Display for SpawnNode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(f, "  SPN {},{}", self.id, self.generator)?;
        Ok(())
    }
}
...
impl Codec for SpawnNode {
    ...
    fn encode<W: io::Write>(&self, output: &mut W) -> io::Result<()> {
        output.write_u8(Self::TAG)?;
        output.write_u64(self.id)?;
        output.write_u64(self.generator)?;
        Ok(())
    }
...
impl fmt::Display for SpawnSink {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "  SNK {},{},{}", self.id, self.source_node, self.group)?;
        Ok(())
    }
}

impl Codec for SpawnSink {
    #[inline]
    fn encode<W: io::Write>(&self, output: &mut W) -> io::Result<()> {
        output.write_u8(Self::TAG)?;
        output.write_u64(self.id)?;
        output.write_u64(self.source_node)?;
        output.write_u64(self.group)?;
        Ok(())
    }
...
impl fmt::Display for FinishSink {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "  FNS {}", self.sink)
    }
}

impl Codec for FinishSink {
    #[inline]
    fn encode<W: io::Write>(&self, output: &mut W) -> io::Result<()> {
        output.write_u8(Self::TAG)?;
        output.write_u64(self.sink)?;
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn finish_sink() {
        check!().with_type::<FinishSink>().for_each(round_trip);
    }
}
