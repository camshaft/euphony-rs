...
impl ModeSteps {
    ...
    fn steps(&self, shift: usize) -> impl Iterator<Item = Ratio<usize>> + '_ {
        let tone_count = self.tone_count;
        self.iter(shift).map(move |i| Ratio::new(*i, tone_count))
    }

    fn intervals(&self, shift: usize) -> impl Iterator<Item = Ratio<usize>> + '_ {
        let tone_count = self.tone_count;
    ...
    fn tones(&self, shift: usize) -> impl Iterator<Item = Ratio<usize>> + '_ {
        let step_count = self.steps.len();
        self.iter(shift)
            .enumerate()
            .flat_map(move |(i, step)| (0..*step).map(move |_| Ratio::new(i, step_count)))
    }
}
